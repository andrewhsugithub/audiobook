...
function chunkTextUltimateFixed(
  text: string,
...
export function startLLMWorker() {
  boss.work<
    AudiobookRequest,
    { bookID: string; ttsUrl: string[]; tags: string[] }
  >("add-tags", async ([job]) => {
    ...
    console.log(
      `Job ${job.id} text has been split into ${chunks.length} chunks.`,
    );
    ...
    for (let i = 0; i < chunks.length; i++) {
      console.log(
        `Processing chunk ${i + 1}/${chunks.length} for job ${job.id}...`,
      );
      ...
      try {
        ...
        console.error(
          `Error processing chunk ${i + 1} for job ${job.id}:`,
          error,
        );
        throw error;
      }
    ...
    console.log(
      `All TTS jobs completed for llm job ${job.id}. Collected URLs: ${ttsUrls.join(", ")}`,
    );
 ...
 * 
{
  "text": "Hello, This is Jana, and welcome to Storynory. This story is dedicated to Adam, Yarden & Neta who asked us to do something just a bit spooky. The Dutch Hotel is not just famous for being haunted. It is also known for the best Dutch pancakes in London. Well, naturally, given its name and history, they had to hire a pancake chef from Holland. And just in case you are wondering, he is a live chef, not a ghost.",
